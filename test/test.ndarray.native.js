...
tape( 'the function returns the last index of an element which equals a provided search element', opts, function test( t ) {
	...
	// Nonnegative stride...
	actual = slastIndexOf( x.length, 1.0, x, 1, 0 );
	t.strictEqual( actual, 1, 'returns expected value' );

	actual = slastIndexOf( x.length-1, 2.0, x, 1, 1 );
	t.strictEqual( actual, 2, 'returns expected value' );

	actual = slastIndexOf( x.length-2, 3.0, x, 1, 2 );
	t.strictEqual( actual, 3, 'returns expected value' );

	actual = slastIndexOf( x.length-2, 4.0, x, 1, 2 );
	t.strictEqual( actual, -1, 'returns expected value' );

	// Negative stride...
	actual = slastIndexOf( x.length, 1.0, x, -1, x.length-1 );
	t.strictEqual( actual, 5, 'returns expected value' );

	actual = slastIndexOf( 3, 2.0, x, -2, x.length-1 );
	t.strictEqual( actual, 1, 'returns expected value' );

	actual = slastIndexOf( 3, 1.0, x, -2, x.length-2 );
	t.strictEqual( actual, 2, 'returns expected value' );

	actual = slastIndexOf( x.length, 4.0, x, -1, x.length-1 );
	t.strictEqual( actual, -1, 'returns expected value' );
...
tape( 'the function returns `-1` if provided `N` parameter is less than or equal to zero', opts, function test( t ) {
	...
	actual = slastIndexOf( 0, 2.0, new Float32Array( [ 1.0, 2.0, 3.0 ] ), 1, 0 ); // eslint-disable-line max-len
	t.strictEqual( actual, -1, 'returns expected value' );

	actual = slastIndexOf( -1, 2.0, new Float32Array( [ 1.0, 2.0, 3.0 ] ), 1, 0 ); // eslint-disable-line max-len
	t.strictEqual( actual, -1, 'returns expected value' );
...
tape( 'the function returns `-1` if provided a search element equal to `NaN`', opts, function test( t ) {
	...
	actual = slastIndexOf( 1, NaN, new Float32Array( [ NaN ] ), 1, 0 );
	t.strictEqual( actual, -1, 'returns expected value' );
